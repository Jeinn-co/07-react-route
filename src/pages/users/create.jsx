...
function saveUserToLocal(newUser) {
  const localUsers = JSON.parse(localStorage.getItem('localUsers') || '[]');
  // 產生一個唯一 id
  newUser.id = `local-${Date.now()}`;
  localUsers.unshift(newUser); // 新的放最前面
  ...
  return (
    <Card title="新增使用者" style={{ maxWidth: 480, margin: '32px auto' }}>
      <Form layout="vertical" onFinish={handleSubmit(onSubmit)}>
        ...
        <Form.Item label="Username" required validateStatus={errors.username ? 'error' : ''} help={errors.username?.message}>
          <Controller
            name="username"
            control={control}
            rules={{ required: '必填' }}
